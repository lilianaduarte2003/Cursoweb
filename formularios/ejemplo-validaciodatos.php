...
<?php
  ...
  // errores que cometemos; //quita espacios
$erroresnombre='';
$errorescorreo='';
...
if (isset($_POST['submit'])) { //isset= establecer ; submit= enviar
   ...
   $email=$_POST['email'];
    ...
    if (!empty($nombre)) {  // si la variable no esta vacia; todos son filtros para no recibir información nociva
        ...
        $erroresnombre .= 'Por favor escribe un nombre <br>'; //si no tenemos varlor en nombre sale mensaje Por favor escribe un nombre
    }
    if (!empty($email)) {  //empty= vacio
        ...
        $errorescorreo .= 'Por favor escribe un correo <br>';
    }
}
